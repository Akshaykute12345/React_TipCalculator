...
function Section() {
  ...
  const getCustomer=()=>{
   ...
   setSum( customerTip.reduce((total,num)=>{
      return total+num;
    }))
...
<div>
          ...
          <span>
            <select name="" onChange={selectChange}>
          ...
          <input type="text" onChange={chooseChange}></input>
          <Button type="button" onClick={getList} class="btn btn-primary" data-bs-toggle="button" autocomplete="off"> <b>ADD CUSTOMER </b>
          </Button> <hr />

          <h6>CUSTOMER LIST </h6>
          <ul>
            { lists.map(elem=>(<li> <h6>{elem.customers} has offered Rs.{elem.tips} Tip.</h6> </li>))
  ...
  <Button class="btn btn-primary" onClick={getCustomer} type="button"><b>CLICK TO CALCULATE TOTAL CUSTOMER AND TIP VALUE</b></Button>
  </div> <hr />
